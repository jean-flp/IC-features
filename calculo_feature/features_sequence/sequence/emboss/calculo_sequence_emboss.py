...
load_dotenv()
__EMAIL__ = os.getenv("EMAIL_EMBOSS")
print(__EMAIL__)
...
def run_pepstats(sequence, email=__EMAIL__, title='elegans'):
...
for pasta in pastas:
    ...
    path_saida = f"{path_data_processed}\\{organismo}\\{file_name.rsplit('.',1)[0]}.emboss.tsv"
    final.to_csv(path_saida, sep = ' ', index = False)
